(function(){
  ...
  function setPressed(sel, test){
    document.querySelectorAll(sel).forEach(function(el){
      var key = el.getAttribute(sel.slice(1, sel.indexOf(']')));
      el.setAttribute('aria-pressed', test(key) ? 'true' : 'false');
    });
  }
  ...
  function render(){
    ...
    if (sum){
      ...
      rows.forEach(function(r){
        ...
        var b = document.createElement('span'); b.className = 'sumprice'; b.textContent = r.price;
  ...
  document.querySelectorAll('[data-pick-clad]').forEach(function(el){
    activate(el, function(){ state.clad = el.getAttribute('data-pick-clad'); render(); });
  });
